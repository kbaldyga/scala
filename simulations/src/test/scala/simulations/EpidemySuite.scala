...
class EpidemySuite extends FunSuite {
  ...
  test("world neightboors is correct") {
    val world = (new EpidemySimulator).createWorld
    val zzNeighboors = world.neighboors(0,0)
    println(zzNeighboors)
    assert(zzNeighboors.exists(s => s.x == 0 && s.y == 7) )
    assert(zzNeighboors.exists(s => s.x == 0 && s.y == 1) )
    assert(zzNeighboors.exists(s => s.x == 1 && s.y == 0) )
    assert(zzNeighboors.exists(s => s.x == 7 && s.y == 0) )
